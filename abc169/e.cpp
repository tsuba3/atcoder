...
int main() {
    ...
    if (n % 2 == 1) {
        // odd
        sort(a.begin(),  a.end(), [](pii lhs, pii rhs) { return lhs.second < rhs.second; });
        const int h = a[a.size() / 2].second;
        sort(a.begin(),  a.end(), [](pii lhs, pii rhs) { return lhs.first < rhs.first; });
        const int l = a[a.size() / 2].first;

        print(h - l + 1);
    } else {
        //even
        sort(a.begin(),  a.end(), [](pii lhs, pii rhs) { return lhs.second < rhs.second; });
        const int h2 = a[a.size() / 2].second + a[a.size() / 2 - 1].second;
        sort(a.begin(),  a.end(), [](pii lhs, pii rhs) { return lhs.first < rhs.first; });
        const int l2 = a[a.size() / 2].first + a[a.size() / 2 - 1].first;
