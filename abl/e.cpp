...
MT f(MT lhs, MT rhs) {
    return MT{(lhs.base * rhs.base) % mod, (rhs.base * lhs.x + rhs.x) % mod};
}

int8_t g(int8_t lhs, int8_t rhs) { return rhs == -1 ? lhs : rhs; }

MT h(MT x, int8_t op, int size) {
    return op == -1 ? x : MT{x.base, (op * one[size]) % mod};
}

using M = Monoid<MT, f>;
using Op = Monoid<int8_t, g>;
using Seg = LazySegmentTree<M, Op, h>;

template<> const MT M::unit({1, 0});
template<> const int8_t Op::unit(-1);
...
int main() {
    ...
    rep(qi, q) {
        int l, r;
        int8_t d;
        io.readui(l), io.readui(r);
        d = io.readc() - '0';
        io.readc();

        seg.update(l - 1, r, d);
        io.writeui(seg.get(0, n).x);
        io.writec('\n');
    }
