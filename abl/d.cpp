...
int f(int lhs, int rhs) { return max(lhs, rhs); }
...
int main() {
    ...
    int n, k;
    io.readui(n), io.readui(k);

    SegmentTree<M> seg(300'001);

    rep(i, n) {
        int a;
        io.readui(a);
        seg.set(a, max(seg.get(a), seg.get(clamp(a - k, 0, 300'001), clamp(a + k, 0, 300'001) + 1) + 1));
    }
