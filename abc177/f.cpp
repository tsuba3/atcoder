...
pii f(pii lhs, pii rhs) {
    return (lhs.second - lhs.first < rhs.second - rhs.first) ? lhs : rhs;
}

using M = Monoid<pii, f>;
template<> const pii M::unit(make_pair(0, INF));


pii g(pii lhs, pii rhs) {
    return make_pair(min(lhs.first, rhs.first), max(lhs.second, rhs.second));
}

using Op = Monoid<pii, g>;
template<> const pii Op::unit(make_pair(INF, 0));


pii hf(const pii m, const pii o, const int n) {
    return make_pair(min(m.first, o.first), max(m.second, o.second));
}
...
int main() {
    ...
    FastIO io(ifs, cout);
    int h, w;
    io.readui(h), io.readui(w);

    LazySegmentTree<M, Op, hf, true> seg(w);
    rep(i, w) {
        seg.v[seg.n + i] = make_pair(i, i);
    }
    seg.build(0, w);

    rep(hi, h) {
        int a, b;
        io.readui(a), io.readui(b);
        a--;

        pii t = seg.get(a);
        seg.update(t.first, b, make_pair(t.first, b == w ? INF : b));

        pii x = seg.get(0, w);
        io.writeui(hi + 1 + x.second - x.first);
        io.writec('\n');
    }
