...
int main() {
    ...
    x[0][0] = M(3).pow(hn * wn - k);
    M p = M(2) / M(3);
    rep(hi, hn) rep(wi, wn) {
        if (hi > 0) {
            if (m[hi - 1][wi] == 'X' || m[hi - 1][wi] == 'D') x[hi][wi] += x[hi - 1][wi];
            if (m[hi - 1][wi] == ' ') x[hi][wi] += p * x[hi - 1][wi];
        }
        if (wi > 0) {
            if (m[hi][wi - 1] == 'X' || m[hi][wi - 1] == 'R') x[hi][wi] += x[hi][wi - 1];
            if (m[hi][wi - 1] == ' ') x[hi][wi] += p * x[hi][wi - 1];
        }
    }

    print(x[hn - 1][wn - 1].value);
