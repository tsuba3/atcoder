...
bool comp(pii a, pii b) {
    int alen = a.second - a.first, blen = b.second - b.first;
    rep(i, min(alen, blen)) {
        char A = s[a.first + i], B = s[b.first + i];
        if (A >= 'a') A -= ('a' - 'A');
        if (B >= 'a') B -= ('a' - 'A');
        if (A < B) return true;
...
int main(){
    ...
    for (int i = 0, m = s.size(); i < m; ++i) {
        int j = i;
        while (s[++i] >= 'a') {}
        ans.emplace_back(make_pair(j, i + 1));
    }

    sort(ans.begin(), ans.end(), comp);
    rep(i, ans.size()) {
        for (int j = ans[i].first; j < ans[i].second; ++j) cout << s[j];
    }
    cout << '\n';
