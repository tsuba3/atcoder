...
int main() {
    ...
    rep(i, m) {
        int a, b, c;
        io.readui(a), io.readui(b), io.readui(c);
        a--, b--;
    ...
    rep(start, n) {
        ...
        while (!q.empty()) {
            ...
            for (auto [v, c] : vec[u]) if (chmin(d[v], d[u] + c)) {
                q.emplace(d[v], v);
            }
        }
    }
    ...
    rep(i, n) {
        rep(j, n) if (i != j) chmin(ans[i], dp[i][j] + dp[j][i]);
        if (ans[i] != INF) io.writeui(ans[i]); else io.writes(string("-1"));
        io.writec('\n');
    }
