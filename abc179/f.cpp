...
int idint(int a, int b) { return a; }
int right_int(int a, int b) {
    return min(a, b);
}

using M = Monoid<int, idint>;
template<> constexpr int M::unit(0);

using Op = Monoid<int, right_int>;
template<> constexpr int Op::unit(INF);

int f(int m, int op, int n) {
    return min(m, op);
}

using Seg = LazySegmentTree<M, Op, f>;
