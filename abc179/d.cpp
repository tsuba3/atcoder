...
using Md = Mod<int64, mod>;

Md addm(Md a, Md b) { return a + b; }
using M = Monoid<Md, addm>;
template<> constexpr Md M::unit(Md(0));
...
int main() {
    ...
    rep(i, n - 1) {
        Md dp = 0;
        rep(ki, k) {
            dp += seg.get(i + 1 - r[ki], i + 1 - l[ki] + 1);
        }
        seg.set(i + 1, dp);
    }

    print(seg.get(n - 1).value);
