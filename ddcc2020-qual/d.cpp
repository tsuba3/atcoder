...
pll add(pll x, pll y) {
    ...
    if (z >= 10) {
        return make_pair(z / 10 + z % 10, x.second + y.second + 2);
    } else {
        return make_pair(z, x.second + y.second + 1);
    }
}

pll pow(pll x, int64 e) {
    if (e == 0) return make_pair(0, 0);
    if (e == 1) return x;
    pll res = pow(add(x, x), e / 2);
    return e & 1 ? add(res, x) : res;
}
...
int main(){
    ...
    rep(i, m) {
        x = add(x, pow(make_pair(d[i], 0), c[i]));
    }
