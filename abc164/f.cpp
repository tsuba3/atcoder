...
int main() {
    ...
    constexpr char SEKI = '0';
    constexpr char WA = '1';
    uint64 n, gyou[510], retu[510];
    char g[510], r[510];

    io.readui(n);
    rep(i, n) g[i] = io.readc(), io.readc();
    rep(i, n) r[i] = io.readc(), io.readc();
    rep(i, n) io.readui(gyou[i]);
    rep(i, n) io.readui(retu[i]);
    ...
    rep(i, n) {
        rep(j, n - 1) io.writeui(ans[i][j]), io.writec(' ');
        io.writeui(ans[i][n - 1]);
        io.writec('\n');
    }
