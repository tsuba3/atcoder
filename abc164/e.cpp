...
struct V {
    int x;
    V(int v, int s) : x(2501 * v + clamp(s, 0, 2500)) {}
    operator int() {
        return x;
    }
    int v() { return x / 2501; }
...
int main() {
    ...
    int n, m, s;
    io.readui(n), io.readui(m), io.readui(s);
    s = clamp(s, 0, 2500);
    vector<pair<V, int>> vec[50*2502];
    vector<int64> dp(n * 2501, INF64);

    rep(mi, m) {
        int u, v, a, b;
        io.readui(u), io.readui(v), io.readui(a), io.readui(b);
        u--, v--;
        rep(i, 2501 - a) {
            vec[V(u, i + a)].emplace_back(V(v, i), b);
            vec[V(v, i + a)].emplace_back(V(u, i), b);
        }
    }

    rep(i, n) {
        int c, d;
        io.readui(c), io.readui(d);
        rep(j, 2500) vec[V(i, j)].emplace_back(V(i, clamp(j + c, 0, 2500)), d);
    }

    using P = pair<int64, V>;
    struct comp {
    ...
    while (!que.empty()) {
        ...
        if (dp[v] < _) continue;
        for (auto k : vec[v]) {
            if (chmin(dp[k.first], dp[v] + k.second)) {
                que.emplace(dp[k.first.x], k.first);
            }
        }
    }
