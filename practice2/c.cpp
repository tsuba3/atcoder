...
// Sigma_i=0^N-1 floor((ai + b) / m)
int64 floor_sum(int64 n, int64 m, int64 a, int64 b) {
    int64 ans = 0, x, y;
    while (true) {
        ans += (a / m) * n * (n - 1) / 2 + (b / m) * (n - 1);
        a %= m, b %= m;
        ...
        x = y * m - b;
        ans += (n - (x + a - 1) / a) * y;
        tie(n, m, a, b) = make_tuple(y, a, m, (a - x % a) % a);
    }
...
int main() {
    ...
    while (t--) {
        int64 n, m, a, b;
        io.readui(n), io.readui(m), io.readui(a), io.readui(b);
        io.writeui(floor_sum(n, m, a, b));
        io.writec('\n');
    }
