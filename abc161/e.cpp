...
int main() {
    ...
    for (int i = n - 1; i > n - c; --i) {
        if (s[i] == 'o') dp[i] = t + 1; else dp[i] = -1;
        cup[i] = max(cup[i + 1], dp[i]);
    }
    for (int i = n - c; i >= 0; --i) {
        if (s[i] == 'o') dp[i] = t + 1; else dp[i] = -1;
        cup[i] = max(cup[i + 1], dp[i]);
    ...
    for (int i = 0; i < n;) {
        if (s[i] == 'x') { i += 1; continue; }
        int x = dp[i];
        if (cup[i + 1] != x) {
            ans[ansi++] = i;
        }
        i += c + 1;
        while (i < n && dp[i] != x - 1) {++i;}
    }


    rep(i, ansi) writeui(ans[i] + 1), writec('\n');
    cout.write(OUT, outi);
