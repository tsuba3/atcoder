...
int main(){
    ...
    int a[1010][1010];
    cin >> n;
    rep(i, n) rep(j, n - 1) { cin >> a[i][j]; a[i][j] -= 1; }

    int it[1010];
    memset(it, 0, sizeof it);
    int ans = 0;
    vector<bool> c(n, false);
    vector<int> q; // 試合があった選手
    vector<int> nt; // 試合をした選手
    q.reserve(n);
    ...
    while (!q.empty()) {
        ...
        nt.reserve(n);
        for (const auto& i : q) {
            int opponent = a[i][it[i]];
            if (!c[i] && !c[opponent] && i == a[opponent][it[opponent]]) {
                c[i] = c[opponent] = true;
                if (it[opponent] < n - 1) nt.push_back(opponent); if (it[i] < n - 1) nt.push_back(i);
            }
        }
        for (const auto& i: nt) {
            it[i] += 1;
