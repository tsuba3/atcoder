...
bool f(bool lhs, bool rhs) { return rhs; }
bool g(bool lhs, bool rhs) { return lhs || rhs; }
bool h(bool x, bool ops, int _) { return x || ops; }

using M = Monoid<bool, f>;
using Op = Monoid<bool, g>;
using Seg = LazySegmentTree<M, Op, h>;

template<> const bool M::unit(false);
template<> const bool Op::unit(false);
...
int main() {
    ...
    int h, w, n, m;
    io.readui(h), io.readui(w), io.readui(n), io.readui(m);

    vector<pii> lamp(n);
    set<int> block_h[1550];
    set<int> block_w[1550];

    rep(i, n) io.readui(lamp[i].first), io.readui(lamp[i].second);

    rep(i, m) {
        int hi, wi;
        io.readui(hi), io.readui(wi);
        block_w[hi].insert(wi);
        block_h[wi].insert(hi);
    }

    rep(i, h + 2) block_w[i].insert(0), block_w[i].insert(w + 1);
    rep(i, w + 2) block_h[i].insert(0), block_h[i].insert(h + 1);

    Seg H[1550], W[1550];

    for (auto [hi, wi] : lamp) {
        // yoko
        auto i1 = block_w[hi].lower_bound(wi);
        auto i2 = i1;
        i1--;
        W[hi].update(*i1 + 1, *i2, true);
        debug3(hi, *i1 + 1, *i2);

        // tate
        auto i3 = block_h[wi].lower_bound(hi);
        auto i4 = i3;
        i3--;
        H[wi].update(*i1 + 1, *i2, true);
        debug3(wi, *i3 + 1, *i4);
    ...
    for (int hi = 1; hi <= h; ++hi) {
        for (int wi = 1; wi <= w; ++wi) {
            bool ok = H[wi].get(hi) || W[hi].get(wi);
            if (ok) ans += 1;
            debug3(hi, wi, H[wi].get(hi));
            debug3(hi, wi, W[hi].get(wi));
        }
    }
