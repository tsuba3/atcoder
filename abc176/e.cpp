...
int main() {
    ...
    int hn, wn, m;
    io.readui(hn), io.readui(wn), io.readui(m);

    unordered_set<pii, phash> s;
    vector<int> H(hn, 0), W(wn, 0);
    s.reserve(m);
    ...
    rep(hi, hn) {
        if (H[hi] == hmax) Hmax.push_back(hi);
        else if (chmax(hmax, H[hi])) Hmax.clear(), Hmax.push_back(hi);
    }
    rep(wi, wn) {
        if (W[wi] == wmax) Wmax.push_back(wi);
        else if (chmax(wmax, W[wi])) Wmax.clear(), Wmax.push_back(wi);
    }

    bool found = false;
    for (int h : Hmax) for (int w : Wmax) {
        if (s.find(make_pair(h, w)) == s.end()) {
            found = true;
            goto loop_end;
        }
    }
    ...
    if (found) io.writeui(hmax + wmax); else io.writeui(hmax + wmax - 1);
    io.writec('\n');
