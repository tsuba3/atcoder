...
int find(int i, int64 x) {
    return lower_bound(begin(s) + i, begin(s) + 2 * n, x + s[i]) - begin(s);
};

bool check(int64 x) {
    rep(i, n) {
        int u = find(i, x); if (u > n + i) continue;
        int v = find(u, x); if (v > n + i) continue;
        int w = find(v, x); if (w > n + i) continue;
        return true;
    }
    return false;
};
